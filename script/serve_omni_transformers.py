"""Host the Qwen3-Omni Thinker model as an HTTP service (transformers).

FALLBACK ONLY: this server loads the model with Hugging Face transformers,
which is slow (RTF ~1 on a B200). Prefer the vLLM-based server
(script/serve_omni.py) for production captioning. This file is kept so the
old behavior remains available if vLLM is not installed.

Usage (on the machine with the model + GPU):
    python -m script.serve_omni_transformers --model-id /path/to/Qwen3-Omni-30B-A3B-Instruct
...
app = FastAPI(title="Omni Captioning Server (transformers)")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Host the Qwen3-Omni Thinker as an HTTP server (transformers, slow)")
